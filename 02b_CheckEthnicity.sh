...
${PLINK}/plink --bfile ${RESULTSDIR}/01/${FILEPREFIX}_QCd \
                ...
                --make-bed \
                --out ${RESULTSDIR}/02/${FILEPREFIX}_QCd_hg38

echo "Update the vairants ID for QCd data---------------------------------------------------------------------------"
# change variant ids to chr:bp since 1000G_gr38_maffilt is chr:bp
awk '{if ($1 != 0) print $2, "chr"$1":"$4}' ${RESULTSDIR}/02/${FILEPREFIX}_QCd_hg38.bim > updateTo1KGFormat.txt
${PLINK}/plink --bfile ${RESULTSDIR}/02/${FILEPREFIX}_QCd_hg38 \
...
echo "Plot the PCs---------------------------------------------------------------------------"
Rscript ${SCRIPTDIR}/4_Resources/plotEthnicity.r ${RESULTSDIR}/02 ${RESULTSDIR}/PCAVariants/${FILEPREFIX}_merged_1000G_forLD ${KGG} 
